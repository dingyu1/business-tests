...
function test_case()
{
    ...
    do
        ...
        then
            let count_tp++
            PRINT_LOG "INFO" "count num of FIBRE type is ${count_fp}"
            if [ ${count_fb} = 1 ]
            then
                PRINT_LOG "INFO" "Get second FIBRE type network card :${network_card} "
                fn_writeResultFile "${RESULT_FILE}" "count_fb_${count_fb}" "pass"
                break 
            fi
    ...
    if [ ${count_tp} = 0 ]
    then
        PRINT_LOG "INFO" "can not get network card type is  TP "
        fn_writeResultFile "${RESULT_FILE}" "count_fb_${count_fb}" "fail"
    fi
