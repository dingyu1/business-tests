...
#用例名称：NIC_PERFORMANCE_007                
#用例功能：内置10G光口1文件传输检查
#作者：fwx654472                            
#完成时间：2019-1-22                        
#前置条件：                                 
#1 D06服务器使用第一个光口连接交换机
#2 交换机与其他同一网段的机器连接          
#测试步骤：                                 
#1 登陆D06服务操作系统
#2 使用scp命令从其他同网段的机器上获取文件
#测试结果：
#1 通过服务器上两个光口可以从内网同一网段的机器获取文件
#######################################################################
#加载公共函数,具体看环境对应的位置修改
...
testcount=3
debug=true
sut_on_board_fiber_00=$env_sut_on_board_fiber_0
#sut_on_board_fiber_010=$env_sut_on_board_fiber_10
#sut_on_board_fiber_020=$env_sut_on_board_TP_20
#sut_on_board_fiber_030=$env_sut_on_board_TP_30
#sut_on_board_fiber_040=$env_sut_external_network_card_40
#sut_on_board_fiber_050=$env_sut_external_network_card_50
tc_on_board_fiber_0=$env_tc_on_board_fiber_0
...
if [ $debug = true ];then
	#sut 本端，tc对端
	sut_on_board_fiber_00=192.168.1.3
	#sut_on_board_fiber_010=192.168.10.3
	#sut_on_board_fiber_020=192.168.20.3
	#sut_on_board_fiber_030=192.168.30.3
	#sut_on_board_fiber_050=192.168.50.11
	
	tc_on_board_fiber_0=192.168.1.6
...
function file_transport_test(){
	...
	PRINT_LOG "INFO" "##########$SCP /root/testfile root@${tc_on_board_fiber_0}:/root/testfile1##########"
    
    $SCP /root/testfile root@${tc_on_board_fiber_0}:/root/testfile1
    if [ $? -eq 0 ]
    then
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_fiber_0}" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_fiber_0}" "fail"
		return 1
    fi
    sync
    sync
	PRINT_LOG "INFO" "##########$SCP root@${tc_on_board_fiber_0}:/root/testfile1 /root/testfile2##########"
	$SCP root@${tc_on_board_fiber_0}:/root/testfile1 /root/testfile2 
    if [ $? -eq 0 ]
    then
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_00" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_00" "fail"
		return 1
    fi
    sync
    sync
...
function ping_check()
{
    for ip in ${tc_on_board_fiber_0}

        do
            ping $ip -c 2
            if [ $? -eq 0 ]
            then
                PRINT_LOG "INFO" "ping $ip success"
            else
                PRINT_LOG "FATAL" "ping $ip fail,please check the network"
            fi
        done
}
...
{
        for i in gcc automake make g++ gcc-c++
        do
        command -v $i
        if [ $? -ne 0 ]
        then
...
function clean_env()
{
	...
	$SSH root@$tc_on_board_fiber_0 "rm -f /root/testfile*"
