...
#用例名称：NIC_PERFORMANCE_005              
#用例功能：内置电口2文件传输检查
#作者：lwx638710                            
#完成时间：2019-1-22                        
#前置条件：                                 
#1 D06服务器使用第二个电口连接交换机
#2 交换机与其他同一网段的机器连接          
#测试步骤：                                 
#1 登陆D06服务操作系统
...
function file_transport_test(){
	...
	PRINT_LOG "INFO" "##########$SCP /root/testfile root@${tc_on_board_TP_30}:/root/testfile1##########"
    
    $SCP /root/testfile root@${tc_on_board_TP_30}:/root/testfile1
    if [ $? -eq 0 ]
    then
        #PRINT_LOG "INFO" "file copy success from port $sut_on_board_TP_30 to ${tc_on_board_TP_30}"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_TP_30}" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
		#PRINT_LOG "INFO" "$SCP /root/testfile root@${tc_on_board_TP_30}:/root/testfile1"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_TP_30}" "fail"
		return 1
    fi
    sync
    sync
    
    PRINT_LOG "INFO" "##########$SCP root@${tc_on_board_TP_30}:/root/testfile1 /root/testfile2 ##########"
	$SCP root@${tc_on_board_TP_30}:/root/testfile1 /root/testfile2 
    if [ $? -eq 0 ]
    then
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_TP_30" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_TP_30" "fail"
		return 1
    fi
    sync
    sync
...
function clean_env()
{
	...
	$SSH root@$tc_on_board_TP_30 "rm -f /root/testfile*"
