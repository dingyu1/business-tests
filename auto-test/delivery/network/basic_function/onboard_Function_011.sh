...
testcount=3
debug=true
sut_on_board_fiber_0=$env_sut_on_board_fiber_0
...
if [ $debug = true ];then
	#sut 本端，tc对端
	sut_on_board_fiber_0=192.168.1.3
...
function file_transport_test(){
    ...
    if [ $? -eq 0 ]
    then
        PRINT_LOG "INFO" "file copy success from port $sut_on_board_fiber_0 to ${tc_on_board_fiber_0}"
    ...
    if [ $? -eq 0 ]
    then
        PRINT_LOG "INFO" "file copy success from port ${tc_on_board_fiber_0} to $sut_on_board_fiber_0"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_0" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_0" "fail"
		return 1
    fi
    sync
    sync
