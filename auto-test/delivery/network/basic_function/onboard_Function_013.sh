...
#用例名称：NIC_PERFORMANCE_013               
#用例功能：内置25G光口2文件传输检查
#作者：fwx654472                            
#完成时间：2019-1-22                        
#前置条件：                                 
#1 D06服务器使用第二个25G光口连接交换机
#2 交换机与其他同一网段的机器连接          
#测试步骤：                                 
#1 登陆D06服务操作系统
...
sut_on_board_fiber_10=$env_sut_on_board_fiber_10
...
tc_on_board_fiber_10=$env_tc_on_board_fiber_10
...
function file_transport_test(){
    ...
    PRINT_LOG "INFO" "##########$SCP /root/testfile root@${tc_on_board_fiber_10}:/root/testfile1##########"
    $SCP /root/testfile root@${tc_on_board_fiber_10}:/root/testfile1
    if [ $? -eq 0 ]
    then
        PRINT_LOG "INFO" "file copy success from port $sut_on_board_fiber_10 to ${tc_on_board_fiber_10}"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_fiber_10}" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_${tc_on_board_fiber_10}" "fail"
		return 1
    fi
    sync
    sync
    
	PRINT_LOG "INFO" "##########$SCP root@${tc_on_board_fiber_10}:/root/testfile1 /root/testfile2##########"
	$SCP root@${tc_on_board_fiber_10}:/root/testfile1 /root/testfile2 
    if [ $? -eq 0 ]
    then
        PRINT_LOG "INFO" "file copy success from port ${tc_on_board_fiber_10} to $sut_on_board_fiber_10"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_10" "pass"
    else
        PRINT_LOG "FATAL" "copy file error, please check your network"
        fn_writeResultFile "${RESULT_FILE}" "copy_file_to_$sut_on_board_fiber_10" "fail"
		return 1
    fi
    sync
    sync
...
function ping_check()
{
    for ip in ${tc_on_board_fiber_10}
...
function clean_env()
{
    ...
    #自定义环境恢复实现部分,工具安装不建议恢复
      #需要日志打印，使用公共函数PRINT_LOG，用法：PRINT_LOG "INFO|WARN|FATAL" "xxx"  
    rm -f /root/testfile*
    #$SSH root@$tc_on_board_fiber_10 "rm -f /root/testfile*"
	$SSH root@$tc_on_board_fiber_10 "rm -f /root/testfile*"
