...
function init_env()
{
    ...
    sshpass -p root ssh root@$ip_board "ip link set $network mtu 9000"
    sshpass -p root ssh root@$ip_board "ethtool -s $network speed 100 duplex full autoneg off"  
...
function clean_env()
{
    ...
    sshpass -p root ssh root@$ip_board "ethtool -s $network speed 1000 duplex full autoneg on"
