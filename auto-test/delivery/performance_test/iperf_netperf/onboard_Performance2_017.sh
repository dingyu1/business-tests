#!/bin/bash
#*****************************************************************************************
# *用例名称：onboard_Performance2_017
# *用例功能：板载enp125s0f0光口大包（10G）
# *作者：wwx573515
# *完成时间：2019-05-16
# *前置条件：
#   1、两台物理机host1和host2，分别用作TAS端和SUT端
#   2、两台物理机板载enp125s0f0光口连接同个交换机
...
function init_env()
{
    ...
    #install
    ip="$env_tc_on_board_fiber_0"
    fn_install_pkg "gcc make tar wget sshpass net-tools" 2
    ip_board=$env_tc_on_board_fiber_10
    network=`ip route|grep "$env_sut_on_board_fiber_0"|awk '{print $3}'`  
#本端绑中断
