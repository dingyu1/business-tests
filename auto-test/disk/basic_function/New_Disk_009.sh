...
function init_env()
{
    ...
    cp ../../../utils/tools/fio ./. || PRINT_LOG "INFO" "scp fio fail"
    chmod 777 fio
    dmesg --clear
}
...
function test_case()
{
    ...
    do
        ...
        fio -filename=${disk} -ioengine=sync -direct=1 -iodepth=128 -rw=write -bs=4k -size=1G -numjobs=8 -runtime=10 -group_reporting -name=fio_test || ./fio -filename=${disk} -ioengine=sync -direct=1 -iodepth=128 -rw=write -bs=4k -size=1G -numjobs=8 -runtime=10 -group_reporting -name=fio_test
        if [ $? -eq 0 ]
        then
