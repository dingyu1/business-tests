...
{	
 ...
 #安装ethtool
	...
	ethtool -i ethxxx 2> $TMPFILE
	a="`cat $TMPFILE`"
	b="No such device"
	if [[ $a =~ $b ]];then
		fn_writeResultFile "${RESULT_FILE}" "Query_ethxxx_device" "pass"
	else
		fn_writeResultFile "${RESULT_FILE}" "Query_ethxxx_device" "fail"
	fi
