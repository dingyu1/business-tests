...
#使用ping命令查看是否有丢包
    ...
    do    
       for k in $ip_board1
        ...
        if [ $str -eq 0 ];then
             PRINT_LOG "INFO" "not loss situation"
             fn_writeResultFile "${RESULT_FILE}" "$i test_ping_name is not loss" "pass"
        else
             PRINT_LOG "FATAL" "this have packet loss situation"
             fn_writeResultFile "${RESULT_FILE}" "$i test_ping_name is loss" "fail"
...
#ping 命令查看是否丢包
     ...
     do
        ...
        do
         ...
         if [ $str -eq 0 ];then
             PRINT_LOG "INFO" "$j not loss situation"
             fn_writeResultFile "${RESULT_FILE}" "$j test_ping is not loss" "pass"
         else
             PRINT_LOG "FATAL" "$j have loss situation"
             fn_writeResultFile "${RESULT_FILE}" "$j test_ping is loss" "fail"
         fi
